...
class WaveformLiveLayer: CALayer {
  ...
  override func draw(in context: CGContext) {
    ...
    waveformDrawer.draw(waveform: samples, on: context, with: configuration.with(size: bounds.size, style: .striped(.init(color: .white, width: 1, spacing: 5, lineCap: .square))), renderer: renderer)
